-- Migration: Tables pour Food Scanner AI
-- Ready for Supabase copy-paste
...
-- Politiques RLS food_items
CREATE POLICY "Food items visible to authenticated users" ON food_items
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Users can insert food items" ON food_items
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

-- Politiques RLS nutrition_scans
...
('Poulet grillé', 165, 31, 0, 3.6, true, 171477),
('Saumon grillé', 206, 22, 0, 12, true, 175167),
('Baguette', 274, 9, 56, 1.2, true, 172687),
('Yaourt nature', 59, 10, 4, 0.4, true, 170894),
('Oeuf entier', 155, 13, 1.1, 11, true, 173424),
...
-- Fonction nettoyage (optionnelle)
